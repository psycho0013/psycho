...
const DiseasesManager = () => {
    ...
    useEffect(() => {
        ...
        const stored = localStorage.getItem('phy_diseases');
        if (stored) {
            setDiseasesList(JSON.parse(stored));
        } else {
            setDiseasesList(diseases);
        }
    }, []);

    const saveData = (data: Disease[]) => {
        localStorage.setItem('phy_diseases', JSON.stringify(data));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            // Update existing
            const updated = diseasesList.map(d =>
                d.id === editingId ? { ...formData as Disease } : d
            );
            saveData(updated);
        } else {
            // Add new
            ...
            saveData([...diseasesList, newDisease]);
        }
    ...
    const handleDelete = (id: string) => {
        if (confirm('هل أنت متأكد من حذف هذا المرض؟')) {
            saveData(diseasesList.filter(d => d.id !== id));
        }
    };
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="p-4 bg-slate-50 rounded-lg border border-slate-200 max-h-60 overflow-y-auto space-y-2">
                                {(() => {
                                    const stored = localStorage.getItem('phy_treatments');
                                    const availableTreatments = stored ? JSON.parse(stored) : treatments;
                                    return availableTreatments.map((treatment: any) => (
