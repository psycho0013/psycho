...
const TreatmentsManager = () => {
    ...
    useEffect(() => {
        ...
        if (stored) {
            setTreatmentsList(JSON.parse(stored));
        } else {
            setTreatmentsList(treatments);
        }
    }, []);

    const saveData = (data: Treatment[]) => {
        localStorage.setItem('phy_treatments', JSON.stringify(data));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            const updated = treatmentsList.map(t =>
                t.id === editingId ? { ...formData as Treatment } : t
            );
            saveData(updated);
        } else {
            ...
            saveData([...treatmentsList, newTreatment]);
        }
    ...
    const handleDelete = (id: string) => {
        if (confirm('هل أنت متأكد من حذف هذا العلاج؟')) {
            saveData(treatmentsList.filter(t => t.id !== id));
        }
    };
