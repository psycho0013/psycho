...
import { symptoms as defaultSymptoms, symptomCategories, type Symptom, type Severity } from '@/data/symptoms';
...
const StepSymptoms = ({ state, setState, onNext, onPrev }: Props) => {
    ...
    useEffect(() => {
        // Load symptoms from localStorage or use defaults
        const stored = localStorage.getItem('phy_symptoms');
        setSymptoms(stored ? JSON.parse(stored) : defaultSymptoms);
    }, []);
